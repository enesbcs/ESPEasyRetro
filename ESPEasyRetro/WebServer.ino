...
void WebServerInit()
{
  ...
  WebServer.on(F("/blescanner"), handle_blescanner);
  ...
  if ((strcasecmp_P(sCommand.c_str(), PSTR("wifidisconnect")) != 0) && (strcasecmp_P(sCommand.c_str(), PSTR("reboot")) != 0) && (strcasecmp_P(sCommand.c_str(), PSTR("reset")) != 0))
  {
    ...
    for (byte x = 0; x < UNIT_MAX; x++)
    {
      if (Nodes[x].ip[0] != 0)
      {
        ...
        html_TD();
        TXBuffer += getNodeTypeDisplayString(Nodes[x].nodeType);
        html_TD();
        TXBuffer += url;
        html_TD();
...
{
  addCopyButton(value, delimiter, name, "0");
}

// add copy to clipboard button
void addCopyButton(const String &value, const String &delimiter, const String &name, const String &dist)
{
  TXBuffer += "<script>function setClipboard"+dist+"() { var clipboard = '';";
  TXBuffer += F(" max_loop = 100;");
  TXBuffer += F(" for (var i = 1; i < max_loop; i++){");
  TXBuffer += F(" var cur_id = '");
  ...
  //Fix HTML
  TXBuffer += "<button class='button link' onclick='setClipboard"+ dist +"()'>";
  TXBuffer += name;
  TXBuffer += F("</button>");
}
...
#ifdef ESP32
  html_TR_TD_height(30);
  addWideButton(F("blescanner"), F("BLE Scan"), F(""));
  html_TD();
  TXBuffer += F("Scan for BLE devices");
#endif
...
//********************************************************************************
// Web Interface BLE scanner  "ESPEasyRetro"
//********************************************************************************
void handle_blescanner() {
  ...
  sendHeadandTail(F("TmplStd"), _HEAD);
#ifdef ESP32
  TXBuffer += F("<table class='multirow'><TR><TH>Address<TH>RSSI<TH>Name<TH>Appearance</TH><TH>Actions</TH></TR>");
...
//  Serial.println(n);
  if (n == 0)
    TXBuffer += F("No BLE devices found");
  else
  {
    BLEAdvertisedDevice pBLEDev;
    for (int cc = 0; cc < n; ++cc)
    {
      pBLEDev = foundDevices.getDevice(cc);
      html_TR_TD();    
//      Serial.println(pBLEDev.getAddress().toString().c_str());
      TXBuffer += "<div id='mac"+String(cc)+"_1' name='mac"+String(cc)+"_1'>"+String(pBLEDev.getAddress().toString().c_str())+"</div>";
      TXBuffer += "<TD>"+String(pBLEDev.getRSSI())+" dBm";
      TXBuffer += "<TD>"+String(pBLEDev.getName().c_str());
      TXBuffer += "<TD>"+String(pBLEDev.getAppearance())+"<TD>";
      addCopyButton("mac"+String(cc),F(""),F("Copy MAC to clipboard"),String(cc));
    }
  }
  TXBuffer += F("</table>");
#else
  TXBuffer += F("<p>Only ESP32 supported");
#endif
  sendHeadandTail(F("TmplStd"), _TAIL);
  TXBuffer.endStream();
}
    ...
    if (showNodes) {
      ...
      for (byte x = 0; x < UNIT_MAX; x++)
      {
        if (Nodes[x].ip[0] != 0)
        {
          ...
          if (Nodes[x].nodeType) {
            String platform = getNodeTypeDisplayString(Nodes[x].nodeType);
...
void handle_advanced() {
  ...
  if (edit.length() != 0)
  {
    ...
    if (dst_start.isValid()) {
      Settings.DST_Start = dst_start.toFlashStoredValue();
    }
    TimeChangeRule dst_end(dstendweek, dstenddow, dstendmonth, dstendhour, timezone);
    if (dst_end.isValid()) {
      Settings.DST_End = dst_end.toFlashStoredValue();
    }
