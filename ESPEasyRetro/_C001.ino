    ...
    case CPLUGIN_PROTOCOL_SEND:
      {
        if (event->idx != 0)
        {
          ...
          boolean battery_added = false;
#ifdef ESP32 // Ugly patch to forward BLE temp device battery "ESPEasyRetro"
          if (event->sensorType == SENSOR_TYPE_TEMP_HUM) {
            byte DeviceIndex = getDeviceIndex(Settings.TaskDeviceNumber[event->TaskIndex]);
            if (Device[DeviceIndex].ValueCount == 3) {
             url += F("&battery=");
             url += String(UserVar[event->BaseVarIndex + 2]);
             battery_added = true;
            }
          }

  #ifdef USES_P104
   #ifndef BLE_BATTERY_PATCH
    #define BLE_BATTERY_PATCH
   #endif
   if (Settings.TaskDeviceNumber[event->TaskIndex] == 104) { // patch for BLE Flora
     if (Settings.TaskDevicePluginConfig[event->TaskIndex][4] != 0) {
             url += F("&battery=");
             url += String(Settings.TaskDevicePluginConfigLong[event->TaskIndex][1]);
             battery_added = true;
     }
   }
  #endif
